...
public:
    ...
    {
    private:
        std::tr1::hash<DataT*> m_ptrHasher;
        std::tr1::hash<IteratorT> m_iterHasher;
    ...
    typedef std::tr1::unordered_set<Cell, typename Cell::Hasher, typename Cell::Comparer> HashSet;
    typedef std::map<DataT*, std::vector<IteratorT> > Samples;
    typedef std::tr1::unordered_map<Cell, Samples, typename Cell::Hasher, typename Cell::Comparer> HashMap;
    typedef std::tr1::unordered_map<DataT*, HashSet> ObjectCells;
...
public:
    ...
    {
    public:
        typedef std::tr1::unordered_map< DataT*, std::tr1::unordered_map< DataT*,
...
public:
    ...
    typedef std::tr1::unordered_map<Cell, std::vector<IteratorT>, typename Cell::Hasher,
    ...
    // a way to specify that using tr1::unordered_map
...
private:
    void clear()
    {
        for( unsigned i = 0 ; i < m_objectMaps.size() ; ++i )
        {
...
public:
    ...
    void* operator()( void* data )
    {
        ...
        for ( unsigned i = b.m_begin; i < b.m_end; ++i )
        {
...
//    std::cout << m_hashMap.load_factor() << std::endl ;
    ...
    // -> Decrease cell size if it is ( should stay higher than cell thickness )
...
bool SpatialHashMap<DataT, IteratorT, GAABB>::Result::next( Collisions &collisions )
{
    ...
    collisions.clear() ;
    for( unsigned iterIdx = begin ; iterIdx < end ; ++ iterIdx )
    {
        ...
        typename Result::Collisions &collisions ) const
{
          ...
          object2 != samples.end(); ++object2 )
    {
        ...
        for ( unsigned index1 = 0; index1 < object1->second.size(); ++index1 )
        {
            ...
            for ( unsigned index2 = startIndex; index2 < object2->second.size(); ++index2 )
            {
